...
export function buildGraphFromMineru({ contentList, pages = [], name = "PDF 卡片图", lang = "zh", cardTypes = null }){
  ...
  items.forEach((item, idx)=>{
    ...
    if (type === "equation"){
      ...
      cards.push({ id: newId("c"), type: "equation", num: "", title, label: title, text, page: Number.isFinite(item.page_idx) ? item.page_idx + 1 : null, section, bbox: item.bbox ?? null, attach: item.attach ?? null, refs: extractRefs(text), idx });
      return;
    }
