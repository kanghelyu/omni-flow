...
export async function writeTree(root, tree) {
  ...
  await writeFile(treePath(root), `${JSON.stringify(tree, null, 2)}\n`, "utf8");
  return tree;
}
...
export async function createFolder(root, path) {
  ...
  if (!folder) throw new Error("folder path cannot be empty");
...
export async function renameFolder(root, path, newPath) {
  ...
  if (!from) throw new Error("source folder path cannot be empty");
  if (!to) throw new Error("target folder path cannot be empty");
  if (from === to) return { ok: true, from, to };
  if (to.startsWith(from + "/")) throw new Error("a folder cannot be moved into one of its own subfolders");
  const tree = await readTree(root);
  if (!tree.folders.includes(from)) throw new Error(`folder does not exist: ${from}`);
...
export async function deleteFolder(root, path) {
  const folder = normFolder(path);
  if (!folder) throw new Error("the root folder cannot be deleted");
  const tree = await readTree(root);
  if (!tree.folders.includes(folder)) throw new Error(`folder does not exist: ${folder}`);
  if (tree.folders.some((p) => p.startsWith(folder + "/"))) throw new Error("folder is not empty (it contains subfolders); delete or move them first");
...
export async function moveGraph(root, graphId, folder) {
  ...
  if (!graphId) throw new Error("missing graphId");
  const tree = await readTree(root);
...
/** WORKLOG.txt: top-level ledger mapping graph → folder.
    Agents must read this (or of_tree) before creating: reuse existing themed folder; new domain → new folder; user's explicit path always wins. */
export async function updateLedger(root) {
  ...
  const lines = [
    "OmniFlow Work Log (auto-generated, do not edit manually)",
    `Updated: ${stamp}`,
    ""
  ];
  ...
  if (rootGraphs.length) {
    lines.push("📁 (root · unfiled)");
    for (const g of rootGraphs) lines.push(`   └ ${g.name}（${g.id}）`);
  }
  lines.push(
    "",
    "Filing rules: agents must read this log before creating graphs — reuse existing themed folders; create new folders only for genuinely new domains; user-specified paths always take precedence."
  );
  await writeFile(join(root, "WORKLOG.txt"), lines.join("\n") + "\n", "utf8");
  return { ok: true, ledger: join(root, "WORKLOG.txt") };
}
