...
/**
 * Interactive adds must point at things that exist: silently accepting a link to a missing graph or
 * node leaves junk in the store that only surfaces later as a "broken link". Bulk import stays
 * permissive on purpose (the map may reference books that have not been imported yet).
 */
export async function assertLinkTargets(root, { fromGraph, fromNode, toGraph, toNode }){
  const { loadGraph } = await import("./graph-service.mjs");
  for (const [side, g, n] of [["from", fromGraph, fromNode], ["to", toGraph, toNode]]){
    let graph;
    try { ({ graph } = await loadGraph(root, g)); }
    catch {
      const error = new Error(`${side} graph does not exist: ${g}`);
      error.code = "XLINK_TARGET";
      throw error;
    }
    if (!(graph.nodes ?? []).some((node) => node.id === n)){
      const error = new Error(`${side} node does not exist in ${g}: ${n}`);
      error.code = "XLINK_TARGET";
      throw error;
    }
  }
