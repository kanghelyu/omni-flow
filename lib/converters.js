...
export function fromMermaid(text, { lang = "en" } = {}) {
  const nodes = new Map();
  const edges = [];
  ...
  const graph = normalizeGraph({
    lang,
    name: L(lang, "导入的 Mermaid 图", "Imported Mermaid map"),
    direction,
...
/** Output text follows the graph's language; English is the documented default. */
function L(lang, zh, en){ return lang === "zh" ? zh : en; }
function typeName(def, lang, fallback){ return (lang === "zh" ? def?.label : def?.labelEn) ?? def?.label ?? fallback; }

export function toMarkdownOutline(graph) {
  const lines = [`# ${graph.name}`, ""];
  if (graph.description) lines.push(graph.description, "");
  const lang = graph.lang;
  const n = graph.nodes?.length ?? 0, e = graph.edges?.length ?? 0;
  lines.push(`> ${n} ${L(lang, "节点", n === 1 ? "node" : "nodes")} · ${e} ${L(lang, "连线", e === 1 ? "edge" : "edges")} · ${L(lang, "方向", "direction")} ${graph.direction ?? "TD"}`, "");
  ...
  for (const [type, list] of byType) {
    ...
    lines.push(lang === "zh" ? `## ${typeName(def, lang, type)}（${type}）` : `## ${typeName(def, lang, type)} (${type})`, "");
    for (const node of list) lines.push(`- **${node.label}**${node.note ? ` — ${node.note.split("\n")[0]}` : ""}`);
    lines.push("");
  }
  lines.push(L(lang, "## 连线", "## Links"), "");
  for (const edge of graph.edges ?? []) {
  ...
  if ((graph.groups ?? []).length) {
    lines.push("", L(lang, "## 分组", "## Groups"), "");
    for (const group of graph.groups ?? []) {
      const labels = (group.members ?? []).map((id) => graph.nodes.find((node) => node.id === id)?.label ?? id);
      lines.push(`- **${group.label}**${L(lang, "：", ": ")}${labels.join(L(lang, "、", ", "))}`);
    }
  }
  return lines.join("\n");
}
...
export async function fromAgentFlow(flowJson, { lang = "en" } = {}) {
  const kindToType = { input: "data", agent: "task", mapAgent: "task", condition: "decision", merge: "process", output: "end" };
  const branchLabel = { true: L(lang, "是", "yes"), false: L(lang, "否", "no"), and: "AND", or: "OR", not: "NOT", nand: "NAND", nor: "NOR", xor: "XOR", xnor: "XNOR" };
  ...
  return normalizeGraph({
    lang,
    name: flowJson.name ?? L(lang, "AgentFlow 导入", "AgentFlow import"),
    description: L(lang, `从 agent-flow 工作流导入（${new Date().toISOString().slice(0, 10)}）`,
                      `Imported from an agent-flow workflow (${new Date().toISOString().slice(0, 10)})`),
    nodes, edges,
...
export function toPlainText(graph) {
  ...
  if (graph.description) lines.push(graph.description);
  lines.push("", L(lang, "[节点]", "[Nodes]"));
  for (const n of graph.nodes) {
    const typeTag = lang === "zh" ? `（${n.type}）` : ` (${n.type})`;
    lines.push(`- ${n.icon ? n.icon + " " : ""}${n.label}${typeTag}${n.note ? " — " + String(n.note).split("\n")[0] : ""}`);
  }
  lines.push("", L(lang, "[关系]", "[Links]"));
  for (const e of graph.edges) {
    lines.push(`- ${label(e.source)} --${e.label || e.type || L(lang, "关联", "related")}--> ${label(e.target)}`);
  }
  for (const g of graph.groups ?? []) {
    const members = (g.members ?? []).map((id) => label(id)).join(L(lang, "、", ", "));
    lines.push("", `${L(lang, "[分组]", "[Groups]")} ${g.label}${L(lang, "：", ": ")}${members}`);
  }
  return lines.join("\n");
}
