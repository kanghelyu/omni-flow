...
<div id="app">
  <aside id="side">
    ...
    <input id="search" data-zh-ph="搜索标题 / 备注 / 标签…" data-en-ph="Search title / note / tag…">
    <div id="filters"></div>
    <div id="list"></div>
  </aside>
  <main id="main">
    ...
    <div id="legend"></div>
    <div id="tools">
      <button data-layout="layered" data-zh="分层" data-en="Layered"></button>
      <button data-layout="clusters" data-zh="聚簇" data-en="Clusters"></button>
      <button data-layout="force" data-zh="力导向" data-en="Force"></button>
      <button data-layout="grid" data-zh="网格" data-en="Grid"></button>
      <button id="theme" data-zh="主题" data-en="Theme"></button>
      <button id="fit" data-zh="适应" data-en="Fit"></button>
    </div>
    <div id="zoom"><button id="zin">+</button><button id="zout">−</button></div>
  </main>
  <aside id="detail"><div class="hdr" id="dTitle" data-zh="节点详情" data-en="Node detail"></div><div class="body" id="dBody"><div class="empty" data-zh="点击画布中的节点查看备注（支持 $公式$）。" data-en="Click a node to read its note (supports $math$)."></div></div></aside>
</div>
...
(function(){
  ...
  var EN = String(navigator.language || '').indexOf('zh') !== 0;
  function T(zh, en){ return EN ? en : zh; }
  function typeLabel(t){ var d = (G.nodeTypes||{})[t] || {}; return (EN && d.labelEn) || d.label || t; }
  ...
  function normalizeLatex(src){
    ...
    t = t.replace(/\\begin\{(tikzpicture|figure\*?|table\*?|lstlisting)\}[\s\S]*?\\end\{\1\}/g, function(_m, env){ return '[' + env + ': ' + T('网页不支持该环境，此处省略', 'this environment is not supported on the web') + ']'; });
  ...
  function renderNote(host, text){
    if (!host) return;
    host.innerHTML = '';
    if (!String(text||'').trim()){ host.innerHTML = '<div class="empty">' + T('（无备注）', '(no note)') + '</div>'; return; }
  ...
  function showDetail(id){
    ...
    if (!id){ dT.textContent = T('节点详情', 'Node detail'); dB.innerHTML = '<div class="empty">' + T('点击画布中的节点查看备注（支持 $公式$）。', 'Click a node to read its note (supports $math$).') + '</div>'; return; }
    var n = nodeIndex[id] || {};
    ...
    renderNote(document.getElementById('dNote'), note || ('*' + T('（该节点暂无备注）', '(this node has no note yet)') + '*'));
  }
  ...
  function renderList(){
    ...
    if (!shown) list.innerHTML = '<div class="empty">' + T('没有匹配的节点', 'No matching nodes') + '</div>';
  }
  function renderFilters(){
    ...
    box.innerHTML = '<button data-t="" class="' + (filter ? '' : 'on') + '">' + T('全部 ', 'All ') + G.nodes.length + '</button>' +
  ...
  document.getElementById('meta').textContent = T(G.nodes.length + ' 节点 · ' + G.edges.length + ' 连线 · ' + (G.groups||[]).length + ' 分组',
    G.nodes.length + (G.nodes.length === 1 ? ' node · ' : ' nodes · ') + G.edges.length + (G.edges.length === 1 ? ' edge · ' : ' edges · ') + (G.groups||[]).length + (G.groups && G.groups.length === 1 ? ' group' : ' groups')) + (G.description ? ' · ' + G.description : '');
  // Apply the language to the static markup (data-zh / data-en) and to the tab title.
  document.title = (G.name ? esc(G.name) : '') + T(' — OmniFlow 画布', ' — OmniFlow canvas');
  document.querySelectorAll('[data-zh]').forEach(function(el){ el.textContent = EN ? el.dataset.en : el.dataset.zh; });
  document.querySelectorAll('[data-zh-ph]').forEach(function(el){ el.placeholder = EN ? el.dataset.enPh : el.dataset.zhPh; });
