...
export function checkLatex(text, { displayMode = false } = {}){
  ...
  // Custom macro definitions are rejected outright: KaTeX would happily compile \newcommand
  // inside THIS fragment, but the macro exists nowhere else — every other card breaks.
  // Agents like to define private symbols; the rule is: standard KaTeX commands only.
  const macroDef = text.match(/\\(newcommand|renewcommand|providecommand|DeclareMathOperator|gdef|edef|xdef|def|let)\b/);
  if (macroDef){
    return {
      total: 1, ok: 0, fragments: 1,
      failed: [{
        fragment: macroDef[0],
        error: "custom macro definition",
        hint: "Custom macro definitions (\\newcommand, \\def, \\let, …) are not allowed: a macro defined in one card does not exist in the others. Write standard KaTeX commands directly (\\mathbb{R}, \\operatorname{Tr}, …).",
        normalized: macroDef[0],
      }],
    };
  }
  let total = 0, ok = 0;
