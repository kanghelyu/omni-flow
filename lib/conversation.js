...
export function setHead(g, nodeId){
  if (!nodeOf(g, nodeId)) throw new Error(`node does not exist: ${nodeId}`);
...
export function mergeBranches(g, { sources, label = "汇合", text = "", speaker = "user", x = null, y = null }){
  ...
  if (nodes.length < 2) throw new Error("merging needs at least 2 source branches");
...
export function aggregateBranches(g, { sources, strategy = "majority", label = null, winner = null, text = "", agent = "judge" }){
  ...
  if (nodes.length < 2) throw new Error("aggregating needs at least 2 branches");
  const votes = new Map();
