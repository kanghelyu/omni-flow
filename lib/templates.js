...
export function buildTemplate(templateId, name, lang = "en") {
...
export function templateSummaries(lang = "en") {
  const en = lang === "en";
...
export async function saveCustomTemplate(root, def) {
  ...
  if (!TPL_ID_RE.test(id)) throw new Error(`invalid templateId (2-48 chars: letters/digits/-/_): ${id}`);
  const nodes = Array.isArray(def.nodes) ? def.nodes : [];
  if (nodes.length === 0) throw new Error("a template needs at least one node");
  if (!String(def.name ?? "").trim()) throw new Error("a template needs a name");
  const record = {
    id,
...
export async function deleteCustomTemplate(root, id) {
  if (!TPL_ID_RE.test(String(id))) throw new Error(`invalid templateId: ${id}`);
...
/** 建图入口：先查内置模板，再查自定义模板。 */
export async function buildTemplateById(root, templateId, name, lang = "en") {
  const en = lang === "en";
  ...
  if (!custom) throw new Error(`template does not exist: ${templateId}`);
