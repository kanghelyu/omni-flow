...
const TOOLS = [
  ...
  { name: "of_export", description: "Export the graph: mermaid / dot (Graphviz) / md (Markdown outline) / txt (plain-text outline for sharing) / json / html (standalone offline interactive canvas, one double-clickable file; large — write the returned text to a .html file)", inputSchema: { type: "object", required: ["id"], properties: { id: { type: "string" }, format: { type: "string", enum: ["mermaid", "dot", "md", "txt", "json", "html"] } } },
    run: async (args) => {
      const root = rootHome();
      const { graph } = await loadGraph(root, args.id);
      const format = args.format ?? "mermaid";
      if (format === "html"){
        // Standalone offline canvas with KaTeX inlined. Done here (not by the HTTP
        // route) because it needs the vendored assets and the node notes from disk.
        const { toStandaloneHtml } = await import("./export-html.js");
        const notes = {};
        for (const n of graph.nodes){
          try { const r = await readNodeNote(root, args.id, n.id); if (r?.exists) notes[n.id] = { content: r.content }; } catch { /* node without a note */ }
        }
        return { format, text: await toStandaloneHtml(graph, notes, { embed: true }) };
      }
      // An unrecognised format used to fall through to Mermaid, so asking for html (or
      // mistyping a format) returned Mermaid text while reporting success. Refuse loudly.
      if (!["mermaid", "dot", "md", "txt", "json"].includes(format)){
        throw new Error(`unsupported export format: ${format} (expected mermaid / dot / md / txt / json / html)`);
      }
  ...
  { name: "of_import_mermaid", description: "Import Mermaid flowchart text as a new graph. What actually survives: card ids, labels and shapes. What does NOT: every node arrives with type \"process\" (Mermaid cannot express OmniFlow node types, so all cards share one colour), Mermaid edge labels become plain edge labels rather than edge types, and classDef / class / style / subgraph / direction lines are dropped — no styling and no groups. Good for a quick first pass; for typed, styled, grouped bulk creation prefer of_import_json, or run of_import_mermaid then of_patch_node / of_patch_node_type to set types.", inputSchema: { type: "object", required: ["text"], properties: { text: { type: "string" }, name: { type: "string" } } },
    run: async (args) => {
  ...
  { name: "of_import_json", description: "Import a full OmniFlow graph JSON object as a new graph — the route that preserves everything: node types, edge types, custom type registries (nodeTypes/edgeTypes), shapes, colours, groups and the direction. Use this for typed bulk creation (>8 cards); unlike of_import_mermaid it loses nothing. Pass the same object shape as graph.json: { name, direction, nodeTypes?, edgeTypes?, nodes: [{id,type,label,x,y,w?,h?,shape?,fill?,...}], edges: [{id?,source,target,type?,label?}], groups?: [{id,label,color,members}], notes? }.", inputSchema: { type: "object", required: ["data"], properties: { data: { type: "object" }, name: { type: "string" } } },
    run: async (args) => {
