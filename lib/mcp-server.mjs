...
const TOOLS = [
  ...
  { name: "of_create_folder", description: "新建文件夹（a/b 自动补齐祖先层级）", inputSchema: { type: "object", required: ["path"], properties: { path: { type: "string", description: "文件夹路径，如 数学/论文笔记" } } },
  ...
  { name: "of_create_graph", description: "按模板创建新图。可指定 folder 归档到某文件夹（自动创建层级）；结果永远包含完整存储位置信息，agent 必须如实转述给用户", inputSchema: { type: "object", required: ["name"], properties: { name: { type: "string" }, template: { type: "string", description: "模板 id，默认 blank" }, description: { type: "string" }, folder: { type: "string", description: "归档文件夹路径（a/b 可嵌套），缺省为根目录" } } },
    run: async (args) => {
      const root = rootHome();
      const graph = await buildTemplateById(root, args.template ?? "blank", args.name);
  ...
  { name: "of_layout", description: "自动布局：mode=layered 按依赖分层（默认）/ mode=clusters 按分组聚簇（同组空间聚在一起）", inputSchema: { type: "object", required: ["id"], properties: { id: { type: "string" }, mode: { type: "string", enum: ["layered", "clusters"] } } },
    run: async (args) => {
      const root = rootHome();
      const { graph } = await loadGraph(root, args.id);
