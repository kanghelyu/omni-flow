...
import { addCrosslink, removeCrosslink, crosslinksForGraph, parseCrosslinkTable } from "./crosslinks.js";
...
async function mutateGraph(id, mutator, { bump = true } = {}) {
  ...
  {
    const { guardGraphText, normalizeGraphText } = await import("./latex.js");
...
const TOOLS = [
  ...
  { name: "of_import_doc", description: "从 PDF 结构化提取结果建卡片图：MinerU 的 content_list.json（推荐，含公式 LaTeX 与页码）或 Markdown。自动识别定义/定理/命题/引理/推论/例/记，抽取编号互引并连依赖边，可挂载原书页面图。", inputSchema: { type: "object", required: ["file"], properties: { file: { type: "string", description: "content_list.json 或 .md 的绝对路径" }, name: { type: "string" }, folder: { type: "string" }, lang: { type: "string", enum: ["zh", "en"] } } },
    run: async (args) => {
      ...
      if (/\.py$/i.test(args.file)){
        ...
        built = buildGraphFromCards(data, { name: args.name ?? "卡片图", lang: args.lang === "zh" ? "zh" : "en" });
        built.__flow = { data };
      } else if (/\.json$/i.test(args.file)){
        built = buildGraphFromMineru({ contentList: JSON.parse(raw), name: args.name ?? "PDF 卡片图", lang: args.lang === "zh" ? "zh" : "en" });
      } else {
        built = buildGraphFromMarkdown({ markdown: raw, name: args.name ?? "Markdown 卡片图" });
      }
      ...
      // 导入是 LaTeX 最大来源：规范化 + 校验（把示例源码放代码围栏即可豁免）
      {
        ...
        if (g.failed.length){
          ...
          throw new Error(`导入内容含 ${g.failed.length} 处无法编译的 LaTeX，已中止（避免落库后显示不出来）：\n${det}`);
        }
      }
      ...
      if (built.__flow?.data?.SECTION_FLOW?.length){
        const ov = buildOverviewFromFlow(built.__flow.data, { name: `${built.graph.name} · 章节总览` });
        ov.graph.id = makeGraphId(ov.graph.name);
        await saveGraph(graphDirSafe(root, ov.graph.id), ov.graph);
        if (args.folder){ try { await moveGraph(root, ov.graph.id, args.folder); } catch { /* 略 */ } }
  ...
  /* ---------- 实时对话记录：开启后每轮自动落图 ---------- */
  { name: "of_live_start", description: "开启非线性对话记录（用户说「开启非线性对话」时调用）。若已有活动会话则续记，否则新建一张对话图。之后每一轮只需调用 of_live_log，不必再传 id。", inputSchema: { type: "object", properties: { topic: { type: "string", description: "本次对话主题（新建时用）" }, folder: { type: "string", description: "归档文件夹" }, lang: { type: "string", enum: ["zh", "en"] }, reuse: { type: "boolean", description: "是否续用已有活动会话（默认 true）" } } },
  ...
  { name: "of_live_log", description: "记录一轮对话（用户发言 role=user，你的回复 role=agent）。开启记录后**每一轮都必须调用**，无需传 id。branchFrom 可显式从历史某节点分叉（非线性）。", inputSchema: { type: "object", required: ["text"], properties: { role: { type: "string", enum: ["user", "agent", "system"], description: "谁说的（缺省 agent）" }, text: { type: "string" }, name: { type: "string", description: "可选的发言人名（多 agent 场景）" }, from: { type: "string", description: "从该节点分叉（缺省接在 head 之后）" }, type: { type: "string" }, status: { type: "string", enum: ["done", "running", "pending", "waiting-human", "failed"] }, handoffTo: { type: "string" }, id: { type: "string", description: "显式指定图 id（缺省用活动会话）" } } },
  ...
  { name: "of_live_stop", description: "停止记录（图保留，可随时回看或续记）。用户说「停止记录 / 结束记录」时调用。", inputSchema: { type: "object", properties: { id: { type: "string" } } },
    run: async (args) => liveStop(rootHome(), { id: args.id ?? null }) },
  { name: "of_live_status", description: "查看当前是否在记录、已记多少轮、当前点头与开放分支。", inputSchema: { type: "object", properties: {} },
    run: async () => liveStatus(rootHome()) },
  /* ---------- 跨图链接 + 分层投影 ---------- */
  { name: "of_xlink_add", description: "新增跨图链接：from=提供依据方，to=使用方（箭头恒为 from→to）。why 必填数学理由，便于日后回看为什么有关联。", inputSchema: { type: "object", required: ["fromGraph", "fromNode", "toGraph", "toNode", "why"], properties: { fromGraph: { type: "string" }, fromNode: { type: "string" }, toGraph: { type: "string" }, toNode: { type: "string" }, why: { type: "string", description: "数学理由（不要写「有相关内容」）" }, kind: { type: "string" } } },
    run: async (args) => addCrosslink(rootHome(), args) },
  { name: "of_xlink_list", description: "列出跨图链接；传 graphId 只看与该图相关的，并标注视角（provides 外部引用 / uses 跨图依据）。", inputSchema: { type: "object", properties: { graphId: { type: "string" } } },
    run: async (args) => ({ links: args.graphId ? await crosslinksForGraph(rootHome(), args.graphId) : await readAllLinks(rootHome()) }) },
  { name: "of_xlink_rm", description: "删除一条跨图链接", inputSchema: { type: "object", required: ["id"], properties: { id: { type: "string" } } },
    run: async (args) => removeCrosslink(rootHome(), args.id) },
  { name: "of_xlink_import", description: "导入跨图链接表（crosslinks.py 或 JSON）", inputSchema: { type: "object", required: ["file"], properties: { file: { type: "string" }, map: { type: "object", description: "书代号 → 本地图 id" } } },
    run: async (args) => {
      const { readFile: rf } = await import("node:fs/promises");
  ...
  { name: "of_project_overview", description: "生成「章节总览」图：节点=小节，边=小节间聚合依赖（权重=条数）。适合先看全局再下钻。", inputSchema: { type: "object", required: ["id"], properties: { id: { type: "string" }, name: { type: "string" }, folder: { type: "string" } } },
    run: async (args) => {
      const root = rootHome();
      ...
      if (args.folder){ try { await moveGraph(root, r.graph.id, args.folder); } catch { /* 归档失败不阻断 */ } }
      return { id: r.graph.id, name: r.graph.name, sections: r.graph.nodes.length, edges: r.graph.edges.length, crossLinks: r.crossLinks };
    } },
  { name: "of_project_sections", description: "按小节拆分：每个小节一张图，本节卡片 + 幽灵卡（跨节/跨书外部依赖的替身，带 open:<图> 可跳回）。切分后跨节联系不丢。", inputSchema: { type: "object", required: ["id"], properties: { id: { type: "string" }, minCards: { type: "number" }, only: { type: "string" }, folder: { type: "string" } } },
