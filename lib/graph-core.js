...
export function normalizeGraph(raw) {
  ...
  return {
    ...
    // 内容语言（zh|en）：CLI 的 `--lang`、MCP 的 `lang`、模板与导入都会传它（默认名、模板文案
    // 跟随它），但过去只在入口处用一下、落库时被丢弃 —— 再读回来永远是默认值，文档承诺的
    // "跟随图自身 lang" 因此不成立。这里显式持久化；非法值收敛为 en。
    lang: graph.lang === "zh" ? "zh" : "en",
...
function normalizeNode(node, index, context = {}) {
  ...
  return {
    ...
    // 任务态：todo/doing/done/blocked；agent 运行态：pending/running/waiting-human/failed/aborted
    status: ["todo", "doing", "done", "blocked", "pending", "running", "waiting-human", "failed", "aborted"].includes(raw.status) ? raw.status : null,
    tags: Array.isArray(raw.tags) ? raw.tags.map(String).slice(0, 12) : [],
    // 附件：原书页面图 / PDF / 任意文件 / 外链 —— 学术卡片的核心信息源
    attachments: (Array.isArray(raw.attachments) ? raw.attachments : [])
      .filter((a)=> a && typeof a === "object" && typeof a.src === "string" && a.src.trim())
      .map((a)=> ({
        kind: ["image", "page", "pdf", "file", "url"].includes(a.kind) ? a.kind : "image",
        src: String(a.src).slice(0, 500),
        label: typeof a.label === "string" ? a.label.slice(0, 120) : "",
        page: Number.isFinite(a.page) ? a.page : null,
...
function normalizeGroup(group) {
  ...
  // 组框几何：一旦被用户固定过就持久化，渲染时优先使用，避免由成员坐标反推导致的漂移/变形。
  // 仅保留一份（覆盖式），不累积历史位置。
  const r = raw.rect;
  if (r && typeof r === "object" && [r.x, r.y, r.w, r.h].every((v)=> Number.isFinite(v))) {
    out.rect = {
      x: Math.round(r.x), y: Math.round(r.y),
      w: Math.max(80, Math.round(r.w)), h: Math.max(60, Math.round(r.h)),
    };
  }
  return out;
}
...
export function validateGraph(graph) {
  ...
  for (const node of graph.nodes ?? []) {
    if (!node.id || typeof node.id !== "string") issues.push(`node without an id (index ${graph.nodes.indexOf(node)})`);
    else if (nodeIds.has(node.id)) issues.push(`duplicate node id: ${node.id}`);
    else nodeIds.add(node.id);
  }
  const edgeIds = new Set();
  for (const edge of graph.edges ?? []) {
    if (!edge.id || edgeIds.has(edge.id)) issues.push(`edge id missing or duplicated: ${edge.id ?? "(empty)"}`);
    else edgeIds.add(edge.id);
    if (!nodeIds.has(edge.source)) issues.push(`edge ${edge.id} starts at a node that does not exist: ${edge.source}`);
    if (!nodeIds.has(edge.target)) issues.push(`edge ${edge.id} ends at a node that does not exist: ${edge.target}`);
    if (edge.source === edge.target && nodeIds.has(edge.source)) warnings.push(`edge ${edge.id} is a self-loop (${edge.source} → ${edge.target})`);
  }
  for (const group of graph.groups ?? []) {
    for (const member of group.members ?? []) {
      if (!nodeIds.has(member)) warnings.push(`group ${group.id} references a node that does not exist: ${member}`);
    }
  }
  const cyclic = detectCycles(graph.nodes ?? [], graph.edges ?? []);
  if (cyclic.length > 0) warnings.push(`cycle: ${cyclic.map((cycle) => cycle.join(" → ")).join("; ")}`);
  const connected = reachability(graph.nodes ?? [], graph.edges ?? []);
  for (const orphan of connected.isolated) warnings.push(`isolated node (no edges): ${orphan}`);
...
/** 设置/追加节点附件（MCP / HTTP / CLI 共用） */
export function setNodeAttachments(graph, nodeId, attachments, { mode = "replace" } = {}){
  const node = graph.nodes.find((n)=> n.id === nodeId);
  if (!node) throw new Error(`node does not exist: ${nodeId}`);
  const list = (Array.isArray(attachments) ? attachments : [attachments]).filter(Boolean);
  node.attachments = mode === "append" ? [...(node.attachments ?? []), ...list] : list;
  return node.attachments;
}

/** 固定组框几何（覆盖旧值；传 null 清除，回到自动贴合内容） */
export function setGroupRect(graph, groupId, rect){
  const group = graph.groups.find((g)=> g.id === groupId);
  if (!group) throw new Error(`group does not exist: ${groupId}`);
  if (rect === null || rect === undefined){ delete group.rect; return null; }
  const { x, y, w, h } = rect;
  if (![x, y, w, h].every((v)=> Number.isFinite(v))) throw new Error("rect needs four numbers: x/y/w/h");
  group.rect = { x: Math.round(x), y: Math.round(y), w: Math.max(80, Math.round(w)), h: Math.max(60, Math.round(h)) };
  return group.rect;
}

/** 批量移动节点（一次事务）：拖组/多选拖动用，避免 N 次请求各自落盘导致中间态 */
export function moveNodes(graph, moves){
  const map = new Map((Array.isArray(moves) ? moves : []).map((m)=> [m.nodeId, m]));
  let n = 0;
  for (const node of graph.nodes){
    const m = map.get(node.id);
    if (!m || !Number.isFinite(m.x) || !Number.isFinite(m.y)) continue;
    node.x = Math.round(m.x); node.y = Math.round(m.y);
    n++;
  }
  return n;
}
